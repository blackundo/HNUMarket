...
function CraftButtonLabel({ children, className }: CraftButtonLabelProps) {
  return (
    <span className={cn('group-hover:text-white relative z-10 transition-colors duration-500', className)}>
      {children}
    </span>
  )
}
...
function CraftButtonIcon({ children, className }: CraftButtonIconProps) {
  ...
  return (
    <span className={cn('relative z-0', iconSize, className)}>
      <span
        className={cn(
          'bg-primary absolute inset-0 -z-10 rounded-full transition-transform duration-500 group-hover:scale-[20]',
          iconSize
        )}
      />
      <span
        className={cn(
          'bg-primary text-white group-hover:bg-background group-hover:text-primary relative z-10 flex items-center justify-center rounded-full transition-all duration-500',
          iconSize
        )}
      >
        {children}
  ...
  return (
    <CraftButtonContext.Provider value={{ size }}>
      <Button
        ...
        asChild={asChild}
        className={cn(
          'group bg-background hover:bg-primary dark:hover:border-primary/30 relative cursor-pointer overflow-hidden rounded-full duration-500 hover:shadow-md dark:border dark:border-transparent',
          className
        )}
        {...rest}
      >
